...
./cross_compile_ffmpeg.sh --gcc-cpu-count=16 --build-ffmpeg-shared=y --build-ffmpeg-static=n --disable-nonfree=y --build-intel-qsv=y --compiler-flavors=win64 --enable-gpl=n --high-bitdepth=n --build-dependencies=y
...
cp -R -f ./sandbox/win64/ffmpeg_git_lgpl_shared/bin/ $OUTPUTDIR/shared
